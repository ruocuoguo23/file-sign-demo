...
func (o *OpkClient) Auth(ctx context.Context, opts ...AuthOpts) (*pktoken.PKToken, error) {
	...
	if o.cosP == nil {
		pkt, _, err := o.oidcAuth(ctx, o.signer, o.alg, authOpts.extraClaims)
		if err != nil {
			return nil, err
		}
		o.pkToken = pkt
...
func (o *OpkClient) AuthWithPKRecord(ctx context.Context, opts ...AuthOpts) (*pktoken.PKToken, *discover.PublicKeyRecord, error) {
	authOpts := &AuthOptsStruct{
		extraClaims: map[string]any{},
	}
	for _, applyOpt := range opts {
	...
	// If no Cosigner is set then do standard OIDC authentication
	if o.cosP == nil {
		pkt, pkRecord, err := o.oidcAuth(ctx, o.signer, o.alg, authOpts.extraClaims)
		if err != nil {
			return nil, nil, err
		}
		o.pkToken = pkt
		pkCopy, err := o.pkToken.DeepCopy()
		return pkCopy, pkRecord, err
	}

	// If a Cosigner is set then check that will support doing Cosigner auth
	if browserOp, ok := o.Op.(BrowserOpenIdProvider); !ok {
		return nil, nil, fmt.Errorf("OP supplied does not have support for MFA Cosigner")
	} else {
		redirCh := make(chan string, 1)
...
func (o *OpkClient) oidcAuth(
	...
	alg jwa.KeyAlgorithm,
	extraClaims map[string]any,
) (*pktoken.PKToken, *discover.PublicKeyRecord, error) {
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to instantiate client instance claims: %w", err)
	}
	...
	// at last we get the oidc public key from idToken
	pkRecord, err := o.Op.PublicKeyByToken(ctx, idToken)
	if err != nil {
		return nil, nil, fmt.Errorf("error getting public key from ID Token: %w", err)
	}

	// print the public key
	fmt.Printf("Public Key: %v\n", pkRecord)

	return pkt, pkRecord, nil
}
