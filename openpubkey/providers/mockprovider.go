...
func (m *MockProvider) VerifyIDTokenWithCache(ctx context.Context, idt []byte, cic *clientinstance.Claims, cachePath string) error {
	m.options.VerifierOpts.DiscoverPublicKey = &m.publicKeyFinder //TODO: this should be set in the constructor once we have constructors for each OP
	return NewProviderVerifier(m.Issuer(), m.options.VerifierOpts).VerifyIDTokenWithCache(ctx, idt, cic, cachePath)
}
