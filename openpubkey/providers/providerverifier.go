...
func (v *DefaultProviderVerifier) VerifyIDToken(ctx context.Context, idToken []byte, cic *clientinstance.Claims) error {
	...
	switch alg {
	case gq.GQ256:
		pubKeyRecord, err := v.verifyGQSig(ctx, idt)
		if err != nil {
			return fmt.Errorf("error verifying OP GQ signature on PK Token: %w", err)
		}

		// log the public key record
		log.Printf("OP Public Key Record: %+v", pubKeyRecord)
	case jwa.RS256:
...
func (v *DefaultProviderVerifier) VerifyIDTokenWithCache(ctx context.Context, idToken []byte, cic *clientinstance.Claims, cachePath string) error {
	// Sanity check that if GQCommitment is enabled then the other options
	// are set correctly for doing GQ commitment verification. The intention is
	// to catch misconfigurations early and provide meaningful error messages.
	if v.options.CommitType.GQCommitment {
	...
	case gq.GQ256:
		...
		log.Printf("OP Public Key Record: %+v", pubKeyRecord)
	case jwa.RS256:
		pubKeyRecord, err := v.loadPublicKeyFromCache(cachePath)
		if err != nil {
			return fmt.Errorf("failed to get OP public key: %w", err)
		}
...
func (v *DefaultProviderVerifier) loadPublicKeyFromCache(path string) (*discover.PublicKeyRecord, error) {
	...
	issuer, _ := key.Get("issuer")

	return &discover.PublicKeyRecord{
		PublicKey: pubKey,
		Alg:       fmt.Sprintf("%v", alg),
		Issuer:    fmt.Sprintf("%v", issuer),
	}, nil
}
...
func (v *DefaultProviderVerifier) verifyGQSig(ctx context.Context, idt *oidc.Jwt) (*discover.PublicKeyRecord, error) {
	algStr := idt.GetSignature().GetProtectedClaims().Alg
	if algStr == "" {
		return nil, fmt.Errorf("missing provider algorithm header")
	}
	if algStr != gq.GQ256.String() {
		return nil, fmt.Errorf("signature is not of type GQ")
	}

	origHeaders, err := originalTokenHeaders(idt.GetRaw())
	if err != nil {
		return nil, fmt.Errorf("malformed ID Token headers: %w", err)
	}

	origAlg := origHeaders.Algorithm()
	if origAlg != jwa.RS256 {
		return nil, fmt.Errorf("expected original headers to contain RS256 alg, got %s", origAlg)
	}

	if idt.GetClaims().Issuer == "" {
		return nil, fmt.Errorf("missing issuer in payload: %s", idt.GetPayload())
	}
	if idt.GetClaims().Issuer != v.issuer {
		return nil, fmt.Errorf("issuer of ID Token (%s) doesn't match expected issuer (%s)", idt.GetClaims().Issuer, v.issuer)
	}

	publicKeyRecord, err := v.options.DiscoverPublicKey.ByToken(ctx, v.Issuer(), idt.GetRaw())
	if err != nil {
		return nil, fmt.Errorf("failed to get provider public key: %w", err)
	}

	rsaKey, ok := publicKeyRecord.PublicKey.(*rsa.PublicKey)
	if !ok {
		return nil, fmt.Errorf("jwk is not an RSA key")
	}
	...
	if !ok {
		return publicKeyRecord, fmt.Errorf("error verifying OP GQ signature on PK Token (ID Token invalid)")
	}
	return publicKeyRecord, nil
}
