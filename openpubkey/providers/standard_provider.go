...
func (s *StandardOp) VerifyIDTokenWithCache(ctx context.Context, idt []byte, cic *clientinstance.Claims, cachePath string) error {
	vp := NewProviderVerifier(
		s.issuer,
		ProviderVerifierOpts{
			CommitType:        CommitTypesEnum.NONCE_CLAIM,
	...
	return vp.VerifyIDTokenWithCache(ctx, idt, cic, cachePath)
}
